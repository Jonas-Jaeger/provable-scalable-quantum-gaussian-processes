...
function single_qubit_random_unitary_state_circuit(initial_state::MPS, sites::Vector{Index{Int64}}, qubit::Int64, t::Float64; seed::Int = 42)
    rng = MersenneTwister(seed)  # create a local RNG with the seed
    P_euler = [Pi, Pz, Py, Pz] # Euler decompsoition (include identity for phase)
    rand_coeffs = 2*pi*randn(rng,4)*t
    gates = [Prot(rand_coeffs[i], P_euler[i]) for i in 1:4]
    indices = [[qubit], [qubit], [qubit], [qubit]]
    new_psi =  get_state!(gates,initial_state,sites,indices)
...
function random_rotations_state_circuit(initial_state::MPS, sites::Vector{Index{Int64}}, t::Float64; seed::Int = 42)
    ...
    new_psi = copy(initial_state)
    for i in 1:n
        P_choose = rand(rng, (Px, Py, Pz))
        gates = [Prot(2*pi*randn(rng)*t, P_choose)]
        indices = [[i]]
        new_psi =  get_state!(gates,new_psi,sites,indices)
    end
    new_psi = truncate(new_psi, cutoff = 10^-20)
    return new_psi
end
